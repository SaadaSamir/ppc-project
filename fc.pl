...
generer_test_matrix :-
    generer_csp(5, 0.3, Matrice),              % Generate a CSP matrix with size 5 and density 0.3
...
% Prune domains of unassigned variables based on (X=V)
forward_check_domains(X, V, DomIn, CSP, DomOut, DomainEmpty) :-
    unassigned_vars(DomIn, Unassigned),
    prune_domains(Unassigned, X, V, DomIn, CSP, DomOut, DomainEmpty).

% prune_domains/8
% prune_domains(UnassignedVars, X, V, DomIn, CSP, DomOut, DomainEmpty)
% Iterates over each unassigned variable and filters its domain
...
% filter_domain/6
% filter_domain(Y, D, X, V, CSP, DFiltered)
% Keep only values Vj in D that satisfy binary constraints with (X=V)
...
% Check if (X=VX,Y=VY) satisfies binary constraints
binary_consistent_pair(X,VX,Y,VY,CSP) :-
    (X < Y -> I=X, J=Y; I=Y, J=X),
    nth0(I, CSP, RowI),
...
% unary_consistent(X, V, CSP)
% Checks unary constraint on variable X for value V
unary_consistent(X, V, CSP) :-
    nth0(X, CSP, RowX),
...
% all_assigned(Dom)
% True if every domain in Dom has exactly one value
all_assigned([]).
all_assigned([(_Var, [ _Val ])|T]) :- all_assigned(T).

% dom_to_assignment(Dom, A)
% Convert the domain structure to a list of (Var,Val) pairs
dom_to_assignment([], []).
dom_to_assignment([(X,[V])|T], [(X,V)|A]) :-
    dom_to_assignment(T, A).

% choose_unassigned_var(Dom, X)
% Choose a variable with a domain size > 1
choose_unassigned_var(Dom, X) :-
    member((X,D), Dom),
    length(D, Len),
    Len > 1,
...
% Assign a single value to Var in Dom
...
% Return a list of variables that are not assigned (domain size > 1)
unassigned_vars(Dom, Unassigned) :-
...
% create_initial_domains(N, D, Dom)
% For N variables, each with domain D, create Dom = [(0,D),(1,D),...,(N-1,D)]
create_initial_domains(N, D, Dom) :-
